...
class ReportGenerator:
    ...
    def _generate_executive_summary(self) -> Dict[str, Any]:
        ...
        if module_a and "total_issues" in module_a:
            ...
            if critical > 0:
                critical_findings.append(f"Найдено {critical} критических визуальных проблем")
            if high > 0:
        ...
        if module_b and "task_status" in module_b:
            ...
            if status == "completed":
                ...
                critical_findings.append("Задача не была выполнена")
            elif status == "max_steps_reached":
                summary_points.append(f"Задача не завершена за {steps} шагов")
        ...
        if module_c and "total_issues" in module_c:
            ...
            if critical > 0:
                critical_findings.append(f"Найдено {critical} критических проблем доступности")
            if serious > 0:
        ...
        if module_d and "session_score" in module_d:
            ...
            if score < -0.3:
                critical_findings.append("Негативный эмоциональный опыт пользователя")
            if trend == "declining":
